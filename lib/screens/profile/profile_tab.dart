...
class _ProfileTabState extends State<ProfileTab> {
                                      ...
                                      Container(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Row(
                                              children: [
                                                Icon(
                                                  Icons.api,
                                                  size: 16,
                                                  color: ext.hasApiConfig ? Colors.green : Colors.orange,
                                                ),
                                                const SizedBox(width: 8),
                                                Text(
                                                  'API 서버 설정',
                                                  style: TextStyle(
                                                    fontWeight: FontWeight.bold,
                                                    fontSize: 13,
                                                    color: ext.hasApiConfig ? Colors.green[900] : Colors.orange[900],
                                                  ),
                                                ),
                                                const Spacer(),
                                                InkWell(
                                                  onTap: () => _showApiConfigDialog(context, ext),
                                                  child: Container(
                                                    ...
                                                    child: Row(
                                                      children: [
                                                        Icon(
                                                          ext.hasApiConfig ? Icons.edit : Icons.settings,
                                                          color: Colors.white,
                                                          size: 14,
                                                        ),
                                                        const SizedBox(width: 4),
                                                        Text(
                                                          ext.hasApiConfig ? '수정' : '설정',
                                                          style: const TextStyle(
                                                            color: Colors.white,
                                                            fontSize: 11,
                                                            fontWeight: FontWeight.bold,
                                            ...
                                            const SizedBox(height: 8),
                                            if (ext.hasApiConfig) ...[
                                              _buildApiInfoRow('URL', ext.apiBaseUrl ?? '-'),
                                              const SizedBox(height: 4),
                                              _buildApiInfoRow('Company ID', ext.companyId ?? '-'),
                                              const SizedBox(height: 4),
                                              _buildApiInfoRow('App Key', ext.appKey ?? '-'),
                                              const SizedBox(height: 4),
                                              _buildApiInfoRow(
                                                'Ports',
                                                'HTTP:${ext.apiHttpPort ?? '-'} / HTTPS:${ext.apiHttpsPort ?? '-'}',
                                              ),
                                            ] else
                                              Text(
                                                'API 서버가 설정되지 않았습니다.\n위의 "설정" 버튼을 눌러 API 서버를 구성하세요.',
                                                style: TextStyle(
                                                  fontSize: 11,
  ...
  // 단말번호 API 설정 다이얼로그
  void _showApiConfigDialog(BuildContext context, MyExtensionModel extension) {
    final apiBaseUrlController = TextEditingController(text: extension.apiBaseUrl ?? '');
    final companyIdController = TextEditingController(text: extension.companyId ?? '');
    final appKeyController = TextEditingController(text: extension.appKey ?? '');
    final httpPortController = TextEditingController(text: extension.apiHttpPort?.toString() ?? '80');
    final httpsPortController = TextEditingController(text: extension.apiHttpsPort?.toString() ?? '443');
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Row(
          children: [
            const Icon(Icons.api, color: Color(0xFF2196F3)),
            const SizedBox(width: 8),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('API 서버 설정', style: TextStyle(fontSize: 18)),
                  Text(
                    '단말번호: ${extension.extension}',
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const Expanded(
                      child: Text(
                        '이 단말번호를 사용할 때 여기 설정된 API 서버로 연결됩니다.',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final httpPort = int.tryParse(httpPortController.text.trim()) ?? 80;
              final httpsPort = int.tryParse(httpsPortController.text.trim()) ?? 443;
              
              if (apiBaseUrl.isEmpty || companyId.isEmpty || appKey.isEmpty) {
              ...
              try {
                await DatabaseService().updateMyExtensionApiConfig(
                  id: extension.id,
                  apiBaseUrl: apiBaseUrl,
                  companyId: companyId,
                  appKey: appKey,
                  apiHttpPort: httpPort,
                  apiHttpsPort: httpsPort,
                );
