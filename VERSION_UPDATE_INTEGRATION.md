# 📱 버전 업데이트 기능 통합 완료

## ✅ 통합된 기능

### 1. **LoginScreen 자동 버전 체크** ⭐ NEW
- **로그인 전에도 업데이트 안내 표시**
- LoginScreen 진입 시 자동으로 버전 체크 수행
- Firestore에서 최신 버전 정보 조회
- 업데이트 필요 시 BottomSheet 자동 표시
- 강제 업데이트 시 로그인 차단 가능

### 2. **MainScreen 자동 버전 체크**
- MainScreen 진입 시 자동으로 버전 체크 수행
- Firestore에서 최신 버전 정보 조회
- 업데이트 필요 시 BottomSheet 자동 표시

### 3. **Firestore 버전 정보 설정**
- Collection: `app_config`
- Document: `version_info`
- 현재 설정된 버전 정보:
  ```json
  {
    "latest_version": "1.0.2",
    "minimum_version": "1.0.0",
    "update_message": "새로운 기능이 추가되었습니다!\n\n• 다크모드 지원\n• 성능 개선\n• 버그 수정",
    "force_update": false
  }
  ```

### 4. **앱 버전**
- 현재 앱 버전: `1.0.1` (테스트용)
- Firestore 최신 버전: `1.0.2`
- → **업데이트 안내가 표시됩니다**

## 🚀 동작 흐름

### **흐름 1: 로그인 전 버전 체크** ⭐ NEW
```
1. 앱 실행
   ↓
2. LoginScreen 표시
   ↓
3. initState 실행
   ↓
4. 화면 렌더링 완료 (addPostFrameCallback)
   ↓
5. _checkAppVersion() 호출
   ↓
6. VersionCheckService.checkVersion()
   ↓
7. Firestore에서 version_info 조회
   ↓
8. 버전 비교
   ├─ 최신 버전: 아무 동작 없음
   └─ 업데이트 필요: VersionUpdateBottomSheet 표시
      ↓
      사용자 선택:
      ├─ [업데이트]: Play Store 이동
      ├─ [오늘 하루 보지 않기]: SharedPreferences 저장
      └─ [닫기 X]: BottomSheet 닫기
         ↓
         로그인 진행 가능
```

### **흐름 2: 로그인 후 버전 체크**
```
1. 사용자 로그인
   ↓
2. MainScreen 진입
   ↓
3. initState 실행
   ↓
4. 화면 렌더링 완료 (addPostFrameCallback)
   ↓
5. _checkAppVersion() 호출
   ↓
6. VersionCheckService.checkVersion()
   ↓
7. Firestore에서 version_info 조회
   ↓
8. 버전 비교
   ├─ 최신 버전: 아무 동작 없음
   └─ 업데이트 필요: VersionUpdateBottomSheet 표시
      ↓
      사용자 선택:
      ├─ [업데이트]: Play Store 이동
      ├─ [오늘 하루 보지 않기]: SharedPreferences 저장
      └─ [닫기 X]: BottomSheet 닫기
```

## 📊 UI 예시

### **업데이트 안내 BottomSheet (라이트 모드)**
```
┌─────────────────────────────────┐
│  [📱] 새 버전 업데이트      [X] │
├─────────────────────────────────┤
│  ╔═══════════════════════╗      │
│  ║  현재 버전  →  최신 버전  ║      │
│  ║    1.0.1      1.0.2   ║      │
│  ╚═══════════════════════╝      │
├─────────────────────────────────┤
│  [i] 새로운 기능이 추가되었습니다! │
│                                 │
│  • 다크모드 지원                │
│  • 성능 개선                    │
│  • 버그 수정                    │
├─────────────────────────────────┤
│      [업데이트] 버튼            │
│     [오늘 하루 보지 않기]       │
└─────────────────────────────────┘
```

### **강제 업데이트 (force_update: true)**
```
┌─────────────────────────────────┐
│  [⚠️] 필수 업데이트              │
├─────────────────────────────────┤
│  ╔═══════════════════════╗      │
│  ║  현재 버전  →  최신 버전  ║      │
│  ║    1.0.1      1.0.2   ║      │
│  ╚═══════════════════════╝      │
├─────────────────────────────────┤
│  [⚠️] 필수 업데이트가 필요합니다!  │
├─────────────────────────────────┤
│     [지금 업데이트] 버튼        │
└─────────────────────────────────┘
※ 닫기 버튼 없음, 스와이프로 닫기 불가
```

## 🧪 테스트 시나리오

### **시나리오 1: 선택적 업데이트 (현재 상태)**

**준비:**
- 현재 앱 버전: 1.0.1
- Firestore latest_version: 1.0.2
- force_update: false

**테스트 순서:**
1. 앱 실행 → 로그인 → MainScreen 진입
2. **업데이트 안내 BottomSheet 자동 표시** ✅
3. 우측 상단 **닫기 버튼(X) 확인** ✅
4. **"오늘 하루 보지 않기" 버튼 확인** ✅
5. "오늘 하루 보지 않기" 클릭
6. 앱 재시작
7. **BottomSheet 표시 안 됨 확인** ✅
8. 날짜 변경 후 재시작 시 다시 표시됨

**예상 로그:**
```
🔄 [VERSION CHECK] Current: 1.0.1
🔄 [VERSION CHECK] Latest: 1.0.2
🔄 [VERSION CHECK] Update Available: true
🔄 [VERSION CHECK] Force Update: false
```

### **시나리오 2: 강제 업데이트**

**Firestore 설정 변경:**
```python
# scripts/setup_version_info.py에서 수정
version_data = {
    'latest_version': '1.0.2',
    'minimum_version': '1.0.2',  # ← 현재 버전보다 높게 설정
    'update_message': '중요한 보안 업데이트가 필요합니다.',
    'force_update': True,  # ← 강제 업데이트 활성화
}
```

**테스트 순서:**
1. 스크립트 실행: `python3 scripts/setup_version_info.py`
2. 앱 재시작 → MainScreen 진입
3. **BottomSheet 표시** ✅
4. **닫기 버튼 없음 확인** ✅
5. **스와이프로 닫기 불가 확인** ✅
6. **"지금 업데이트" 버튼만 표시** ✅
7. "지금 업데이트" 클릭 → Play Store 이동 확인

**예상 로그:**
```
🔄 [VERSION CHECK] Current: 1.0.1
🔄 [VERSION CHECK] Latest: 1.0.2
🔄 [VERSION CHECK] Update Available: true
🔄 [VERSION CHECK] Force Update: true
```

### **시나리오 3: 최신 버전 (업데이트 불필요)**

**pubspec.yaml 수정:**
```yaml
version: 1.0.2+13  # Firestore와 동일한 버전
```

**테스트 순서:**
1. 앱 재시작 → MainScreen 진입
2. **BottomSheet 표시 안 됨** ✅

**예상 로그:**
```
🔄 [VERSION CHECK] Current: 1.0.2
🔄 [VERSION CHECK] Latest: 1.0.2
🔄 [VERSION CHECK] Update Available: false
🔄 [VERSION CHECK] Force Update: false
```

## 🔧 Firestore 버전 정보 관리

### **버전 정보 업데이트**

**방법 1: 스크립트 사용 (권장)**
```bash
# scripts/setup_version_info.py 편집
# version_data 수정 후 실행
python3 scripts/setup_version_info.py
```

**방법 2: Firebase Console**
1. Firebase Console 접속
2. Firestore Database 선택
3. `app_config` 컬렉션 → `version_info` 문서 선택
4. 필드 수정:
   - `latest_version`: 최신 버전 번호
   - `minimum_version`: 최소 지원 버전
   - `update_message`: 업데이트 안내 메시지
   - `force_update`: 강제 업데이트 여부

### **버전 관리 Best Practices**

**1. 버전 번호 규칙 (Semantic Versioning)**
- `MAJOR.MINOR.PATCH+BUILD`
- 예: `1.0.2+13`
  - MAJOR (1): 주요 변경
  - MINOR (0): 새 기능 추가
  - PATCH (2): 버그 수정
  - BUILD (13): 빌드 번호

**2. latest_version 업데이트 시점**
- 새 버전 APK를 Play Store에 업로드한 후
- 사용자가 업데이트할 수 있는 상태일 때

**3. minimum_version 설정**
- 심각한 버그 수정 시
- 보안 업데이트 시
- API 변경으로 이전 버전 호환 불가 시

**4. force_update 사용 시점**
- 심각한 보안 취약점 발견 시
- 서버 API 버전 변경으로 이전 클라이언트 동작 불가 시
- 데이터 손실 위험이 있는 버그 수정 시

## 📱 앱 버전 변경 방법

### **테스트용 버전 다운그레이드**
```yaml
# pubspec.yaml
version: 1.0.1+13  # 1.0.2 → 1.0.1
```

### **프로덕션 버전 업그레이드**
```yaml
# pubspec.yaml
version: 1.0.2+13  # 1.0.1 → 1.0.2
```

**변경 후:**
```bash
# APK 빌드 (버전이 자동으로 반영됨)
flutter build apk --release
```

## 🔍 디버깅

### **버전 체크 로그 확인**
```dart
// MainScreen._checkAppVersion()에서 자동 출력
🔄 [VERSION CHECK] Current: 1.0.1
🔄 [VERSION CHECK] Latest: 1.0.2
🔄 [VERSION CHECK] Update Available: true
🔄 [VERSION CHECK] Force Update: false
```

### **Firestore 데이터 확인**
```bash
# Firebase Console
# Firestore Database > app_config > version_info
```

### **SharedPreferences 확인**
```dart
// 오늘 하루 보지 않기 저장 키
version_update_dismissed_date: "2025-12-05"
```

## 🎯 프로덕션 배포 체크리스트

- [ ] **pubspec.yaml 버전을 1.0.2로 복원**
  ```yaml
  version: 1.0.2+14  # Build 번호 증가
  ```

- [ ] **Firestore version_info 확인**
  ```json
  {
    "latest_version": "1.0.2",
    "minimum_version": "1.0.0",
    "update_message": "실제 업데이트 내용",
    "force_update": false
  }
  ```

- [ ] **Play Store URL 확인**
  ```dart
  downloadUrl: 'https://play.google.com/store/apps/details?id=com.olssoo.makecall_app'
  ```

- [ ] **APK 빌드 및 테스트**
  ```bash
  flutter build apk --release
  ```

- [ ] **Play Store 업로드 후 version_info 업데이트**

## 📚 추가 참고 자료

### **핵심 파일**
- **BottomSheet 위젯**: `lib/widgets/version_update_bottom_sheet.dart`
- **사용 가이드**: `lib/widgets/version_update_bottom_sheet_example.md`
- **버전 체크 서비스**: `lib/services/version_check_service.dart`
- **Firestore 설정 스크립트**: `scripts/setup_version_info.py`

### **통합된 화면**
- **LoginScreen**: `lib/screens/auth/login_screen.dart` ⭐ NEW
  - 로그인 전 버전 체크
  - 로그 태그: `[VERSION CHECK - LOGIN]`
- **MainScreen**: `lib/screens/home/main_screen.dart`
  - 로그인 후 버전 체크
  - 로그 태그: `[VERSION CHECK]`

## 🎉 완료!

버전 업데이트 기능이 **LoginScreen과 MainScreen 모두**에 완전히 통합되었습니다!

### **주요 특징**
✅ **로그인 전에도 업데이트 가능** (LoginScreen)
✅ **로그인 후에도 업데이트 가능** (MainScreen)
✅ **강제 업데이트 시 로그인 차단 가능**
✅ **다크모드 최적화 UI/UX**
✅ **오늘 하루 보지 않기 기능**

현재 **앱 버전 1.0.1**로 테스트할 수 있으며,  
프로덕션 배포 전 **1.0.2로 복원**하시면 됩니다.

---

## 🔗 Git Commits

### **버전 체크 기능**
- **`2010f2f`**: LoginScreen에 버전 체크 추가 ⭐ NEW
- **`22a2e57`**: MainScreen에 버전 체크 통합
- **`d933969`**: BottomSheet 위젯 구현
- **`594ab2a`**: 통합 가이드 문서

**Repository**: https://github.com/ringneck/makecall
